...
import {minusPizzaBucketAC, plusPizzaBucketAC} from "../redux/buscetReducer";
...
export function useApp() {
    ...
    const backet2=useSelector<AppStoreType,Basket>(state=>state)
    ...
    const minusPizzaBucket = React.useCallback((_id: string) => {
        ...
        if (idx !== -1) { setBacket(R.remove(idx, 1, backet)) }
    }, [backet]);
